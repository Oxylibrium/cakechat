...
def get_tokenized_test_lines(corpus_name, tokenizer):
...
def load_context_free_val(corpus_name, tokenizer):
    corpus_path = get_processed_corpus_path(corpus_name)
    with open(corpus_path) as file:
        while True:
            x = tokenizer.tokenize_line(file.readline().strip())
            y = tokenizer.tokenize_line(file.readline().strip())
            if x and y:
                yield (x, y)
            else:
                break
...
def load_contextual_set(
    filename, tokenizer, condition_to_idx, subset_size=None
):
    corpus_path = get_processed_corpus_path(filename)

    with io.open(corpus_path) as corpus:
        if subset_size:
...
def load_conditioned_train_set(
    tokenizer, condition_to_index, train_subset_size=TRAIN_SUBSET_SIZE
):
    processed_corpus_path = get_processed_corpus_path(TRAIN_CORPUS_NAME)

    warnings.warn(DeprecationWarning("Replace with load_contextual_set"))

    def load_processed_dialogs_from_json(
        lines, text_field_name, condition_field_name
    ):
    ...
    x_train, y_train, n_dialogs = transform_lines_to_nn_input(
        tokenized_alternated_train_lines, tokenizer
    )
...
def generate_subset(dataset, subset_size, random_seed=RANDOM_SEED):
    ...
    sample_idx = np.random.choice(
        dataset.x.shape[0], size=subset_size, replace=False
    )
    ...
    return Dataset(
        ...
        if dataset.condition_ids is not None
        else None,
    )


# TODO: Figure out where this function is used,
# and investigate why all datasets are loaded in the first place.
def load_datasets(token_to_index, condition_to_index):
    tokenizer = Tokenizer(
        token_to_index, INPUT_SEQUENCE_LENGTH, INPUT_CONTEXT_SIZE
    )
    train = load_conditioned_train_set(tokenizer, condition_to_index)

    context_free_iter = load_context_free_val(
        CONTEXT_FREE_VAL_CORPUS_NAME, tokenizer
    )

    # TODO: convert context_free_iter to x and y
    questions = load_questions_set(token_to_index)

    validation_set_size = validation[0].shape[0]
