...
def get_alternated_dialogs_lines(dialogs):
    for dialog in dialogs:
        for first_dialog_line, second_dialog_line in zip(dialog, dialog[1:]):
            yield first_dialog_line, second_dialog_line


def get_dialog_lines_and_conditions(
        dialog_lines, text_field_name, condition_field_name):
    """
    Splits one dialog_lines generator into two generators:
    one for conditions and one for dialog lines.
    """
